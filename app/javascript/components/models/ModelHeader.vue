<template>
  <div class="flex gap-[8px] items-center mb-5 text-lg text-[#606266] font-semibold md:px-5 flex-wrap">
    <el-avatar :size="24" :src="avatar" class="flex-shrink-0"></el-avatar>
    <a class="cursor-pointer hover:text-[#223B99]" @click="redirect">
      {{ path.split('/')[0] }}
    </a>
    <div>/</div>
    <a class="text-[#303133] max-w-full break-words cursor-pointer hover:text-[#223B99]" href="#">
      {{ path.split('/')[1] }}
    </a>
