<template>
  <div class="max-w-[400px] min-w-[300px] pl-[20px] pr-[40px] md:px-0 sm:px-0 sm:w-full py-[36px]">
    <div>
      <el-avatar :size="160" :src="avatar"> </el-avatar>
    </div>
    <div class="text-[24px] leading-[32px] font-semibold mt-[16px]">{{ displayName }}</div>
    <div class="text-[16px] text-[#909399] max-w-[max-content] leading-[24px] mb-[16px] mt-[4px]">@{{name}}</div>
    <div v-if="isLoggedIn" class="my-[36px]">
      <a href="/settings/profile" class="mr-[10px] px-[12px] py-[5px] border border-[#DCDFE6] rounded-[8px] text-[14px] leading-[22px] font-medium hover:bg-[#EAECF0] hover:text-[#808080]">{{ $t('profile.editProfile')}}</a>
      <a href="/settings/profile" class="px-[12px] py-[5px] border border-[#DCDFE6] rounded-[4px] text-[14px] leading-[22px] font-medium hover:bg-[#EAECF0] hover:text-[#808080] hidden">{{ $t('profile.settings')}}</a>
    </div>
    ...
    <div v-if="isLoggedIn && email"
      ...
      <div class="w-[90%] overflow-hidden text-ellipsis">{{ email }}</div>
    </div>
    <div v-if="isLoggedIn && theLastLoginTime"
         class="mt-[16px] text-[#606266] text-[16px] leading-[24px] flex items-center gap-[8px]">
    ...
    <div class="my-[16px] mt-[36px]">
      ...
      <div v-else class="text-[#909399]">{{ $t('all.noData')}}</div>
    </div>
  </div>
</template>
<script>
