<template>
  <div class="max-w-[400px] pl-[20px] pr-[40px] md:px-0 sm:px-0 sm:w-full py-[36px]">
    <!-- avatar -->
    <div>
      <el-avatar :size="160" :src="userStore.avatar ? userStore.avatar : avatar"></el-avatar>
    </div>
    <!-- nickname -->
    <div class="text-[24px] leading-[32px] font-semibold">
      {{ userStore.nickname ? userStore.nickname : displayName }}
    </div>
    <!-- name -->
    <div class="text-[16px] text-[#909399] max-w-[max-content] leading-[24px] mb-[16px]">
      @{{ userStore.username ? userStore.username : displayName }}
    </div>
    <!-- profile settings -->
    <div v-if="userStore.isLoggedIn ? userStore.isLoggedIn : isLoggedIn" class="flex xl:flex-col md:flex-row gap-3">
    ...
    <div v-if="showPhone"
      ...
      {{ userStore.phone ? userStore.phone : phone }}
    </div>
    <!-- email -->
    <div v-if="showEmail"
         class="mt-[16px] text-[#606266] text-[16px] leading-[24px] flex items-center gap-[8px]">
      ...
      <div class="w-[90%] overflow-hidden text-ellipsis">{{ userStore.email ? userStore.email : email }}</div>
    </div>
    <!-- last login time -->
    <div v-if="showLastLoginTime"
         class="mt-[16px] text-[#606266] text-[16px] leading-[24px] flex items-center gap-[8px]">
      ...
      {{ userStore.lastLoginTime ? userStore.lastLoginTime : theLastLoginTime }}
    </div>
    <!-- organization -->
    <div class="my-[16px] mt-[36px]">
      ...
      <div v-if="userStore.orgs != []" class="flex gap-[10px] flex-wrap mt-[16px]">
        <p v-for="org in userStore.orgs">
          <a :href="`/organizations/${org.path}`">
            <img :src="org.logo" class="rounded-[50%] h-[40px] w-[40px]" />
          </a>
        </p>
      </div>
      <div v-else-if="userOrgs != []" class="flex gap-[10px] flex-wrap mt-[16px]">
...
<script setup>
  ...
  const theLastLoginTime = computed(() => props.lastLoginTime ? props.lastLoginTime.slice(0, -6) : "");
  const userRoles = computed(() => props.roles ? JSON.parse(props.roles) : []);
  const userOrgs = computed(() => props.orgs ? JSON.parse(props.orgs) : []);
  const showPhone =computed(() => {
    if (userStore.isLoggedIn && userStore.phone) {
    return userStore.phone;
    }
    return props.isLoggedIn && props.phone;
  })
  const showEmail = computed(() => {
    if (userStore.isLoggedIn && userStore.email) {
    return userStore.email;
    }
    return props.isLoggedIn && props.email;
  })
  const showLastLoginTime = computed(() => {
    if (userStore.isLoggedIn && userStore.lastLoginTime) {
    return userStore.lastLoginTime;
    }
    return props.isLoggedIn && props.lastLoginTime;
  })
