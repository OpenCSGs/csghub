<template>
  <div class="border border-[#DCDFE6] rounded-[8px] my-[32px] md:my-0 md:border-none px-[24px] py-[24px]">
    <div class="flex flex-col gap-[8px]">
      <div class="flex items-center gap-[20px] text-[20px] leading-[28px] font-semibold"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M14.6663 7.99984C14.6663 9.6665 12.6663 13.3332 7.99967 13.3332C3.33301 13.3332 1.33301 9.6665 1.33301 7.99984C1.33301 6.33317 3.33301 2.6665 7.99967 2.6665C12.6663 2.6665 14.6663 6.33317 14.6663 7.99984Z" stroke="#606266" stroke-linecap="round"/><path d="M9.99967 7.99984C9.99967 9.10441 9.10424 9.99984 7.99967 9.99984C6.89511 9.99984 5.99967 9.10441 5.99967 7.99984C5.99967 6.89527 6.89511 5.99984 7.99967 5.99984C9.10424 5.99984 9.99967 6.89527 9.99967 7.99984Z" stroke="#606266" stroke-linecap="round"/></svg>
        Change model visibility
      </div>
      <div class="max-w-[864px] ml-[36px] text-[14px] text-[#606266] leading-[22px]">This model is currently <span class="text-black font-semibold">【{{visibility}}】</span>. Only you (personal model) or members of your organization (organization model) can see and commit to this model.</div>
    ...
    <div class="flex flex-col gap-[8px]">
      ...
      <div class="ml-[36px] text-[14px] text-[#606266] leading-[22px]">This action <span class="text-black font-semibold">cannot</span> be undone. This will permanently delete the <span class="text-black font-semibold">{{path}}</span> model repository and all its files.</div>
      ...
      <div class="ml-[36px] flex">
        <div @click="del" class="text-[#98A2B3] py-[8px] px-[12px] bg-[#F2F4F7] text-[14px] leading-[20px] rounded-[8px]" :class="delDesc == datasetPath ?'bg-[#D92D20] text-[#FFFFFF] cursor-pointer active:shadow-box active:space-y-0 active:space-x-0 active:ring-4 active:ring-red-400 active:ring-opacity-25 active:bg-[#D92D20] hover:bg-[#B42318] hover:text-white':''">I understand, delete this model</div>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    changeVisibility(value){
      ElMessageBox({
        ...
        cancelButtonText: 'Cancel'}).then((action) => {
        ElMessage({
          type: 'info',
          message: `切换成功`,
        })
