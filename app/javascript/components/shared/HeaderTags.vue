<template>
  <div class="flex items-center flex-wrap gap-4 md:px-5 md:mb-[30px] md:flex-col md:items-start">
    <div v-if="taskTags.length" class="flex flex-wrap items-center gap-2">
      <div class="text-sm text-[#909399]">任务:</div>
      <div v-for="tag in taskTags" class="text-sm text-[#303133] px-[8px] py-[3px] rounded cursor-pointer flex items-center border gap-1" :style="`color: ${tag.color}; border: solid`">
        {{ tag.zh_name || tag.name }}
      </div>
    </div>
    <div v-if="frameworkTags.length" class="flex flex-wrap items-center gap-2">
      <div class="text-sm text-[#909399]">框架:</div>
      <div v-for="tag in frameworkTags">
    ...
    <div v-if="otherTags.length" class="flex flex-wrap items-center gap-2">
      <div class="text-sm text-[#909399]">其他:</div>
      <div v-for="tag in otherTags" class="text-sm text-[#303133] px-[8px] py-[3px] rounded cursor-pointer flex items-center border gap-1">
        {{ tag.name }}
      </div>
    </div>
    <div v-if="licenseTags.length" class="flex flex-wrap items-center gap-2">
      <div class="text-sm text-[#909399]">许可证:</div>
      <div v-for="tag in licenseTags" class="text-sm text-[#303133] px-[8px] py-[3px] rounded cursor-pointer flex items-center border gap-1">
...
<script setup>
  ...
  console.log('otherTags', props.otherTags)
</script>