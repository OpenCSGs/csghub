<template>
  <div class="flex items-center flex-wrap gap-4 md:px-5 md:mb-[30px] md:flex-col md:items-start">
    ...
    <a :href="`/${prefix}?tag=${tag.name}&tag_type=Task`" v-for="tag in taskTags" class="text-sm text-[#303133] px-[8px] py-[4px] rounded cursor-pointer flex items-center gap-1 bg-[#d3d3d354]" :style="`color: ${tag.color}`">
      {{ tag.zh_name || tag.name }}
    </a>
    <div v-if="frameworkTags.length"  class="text-sm text-[#909399]">框架:</div>
    <a :href="`/${prefix}?tag=${tag.name}&tag_type=Framework`" :style="`color: ${tag.color}`" v-for="tag in frameworkTags">
    ...
    <a :href="`/${prefix}?tag=${tag.name}&tag_type=License`" v-for="tag in licenseTags" class="text-sm text-[#303133] px-[8px] py-[4px] rounded cursor-pointer flex items-center border gap-1">
