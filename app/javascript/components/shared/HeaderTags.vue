<template>
  <div class="flex items-center flex-wrap gap-4 md:px-5 md:mb-[30px] md:flex-col md:items-start">
    ...
    <tag-item v-for="tag in theTaskTags" :tag="tag" @handleTagClick="searchByTag" />
    <MoreTags v-if="moreTaskTags" :num="taskTags.length - 3" target="task" @view-more-targets="viewMoreTargets" />
    <!-- Framework -->
    ...
    <MoreTags v-if="moreFrameworkTags" :num="frameworkTags.length - 3" target="framework" @view-more-targets="viewMoreTargets" />
    <!-- Language -->
    <div v-if="languageTags.length" class="flex items-center text-sm text-[#909399]">{{ $t('all.languages') }}:</div>
    <a v-for="tag in theLanguageTags"
    ...
    <MoreTags v-if="moreLanguageTags" :num="languageTags.length - 3" target="language" @view-more-targets="viewMoreTargets" />
    <!-- industryTags -->
    <div v-if="industryTags.length" class="text-sm text-[#909399]">{{ $t('all.industry') }}:</div>
    <div v-for="tag in theIndustryTags" class="text-sm text-[#303133] px-[8px] py-[4px] rounded flex items-center border gap-1">
      {{ this.$i18n.locale === 'zh'? (tag.zh_name || tag.name) : tag.name }}
    </div>
    <MoreTags v-if="moreIndustryTags" :num="industryTags.length - 3" target="industry" @view-more-targets="viewMoreTargets" />
    <!-- Other -->
    <div v-if="otherTags.length" class="text-sm text-[#909399]">{{ $t('all.others') }}:</div>
    <div v-for="tag in theOtherTags" class="text-sm text-[#303133] px-[8px] py-[4px] rounded flex items-center border gap-1">
      {{ tag.name }}
    </div>
    <MoreTags v-if="moreOtherTags" :num="otherTags.length - 3" target="other" @view-more-targets="viewMoreTargets" />
    <!-- License -->
    <div v-if="licenseTags.length" class="text-sm text-[#909399]">License:</div>
    <a v-for="tag in theLicenseTags"
    ...
    <MoreTags v-if="moreLicenseTags" :num="licenseTags.length - 3" target="license" @view-more-targets="viewMoreTargets" />
  </div>
</template>
