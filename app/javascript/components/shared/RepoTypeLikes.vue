<template>
  ...
  <div class="mt-[32px]">
    <h3 class="text-[20px] text-[#303133] flex items-center gap-[8px]">
      ...
      <span>{{ $t(`organization.${repoType}`) }}</span>
    </h3>
    <div v-if="hasRepos" class="grid grid-cols-2 lg:grid-cols-1 gap-4 mb-4 mr-6 mt-[16px]">
      <repo-item v-for="repo in repos.data" :repo="repo" :repo-type="repoType"></repo-item>
    </div>
    <div v-else class="flex flex-wrap gap-4 mb-4 mt-[16px]">
      {{ $t('all.noData') }}
    </div>
    <el-skeleton class="pr-6" v-if="reposLoading" :rows="2" animated />
  </div>
</template>
<script setup>
  ...
  const { t } = useI18n();
  const repos = ref(props.repoList)

  const reposLoading = ref(false)

  const hasRepos = computed(() => props.repoList?.total > 0)
