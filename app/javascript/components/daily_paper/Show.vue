    ...
    <div class="flex md:flex-col max-w-[1280px] px-[20px] m-auto">
      <div class="w-[752px] md:w-full md:border-none pr-[16px] border-r-[1px] border-[#EAECF0]">
        <a :href="backUrl" class="flex gap-[5px] text-[#475467] cursor-pointer hover-filter-primary px-[12px] py-[8px]">
          <span class="w-[20px] h-[20px]"><chevron-left /></span>
          <span class="text-[14px] font-[500] leading-[20px]">{{ $t('dailyPaper.back') }}</span>
        </a>
        ...
        <div class="flex justify-items items-center mt-[20px] text-[#475467] text-[18px] font-[300] leading-[28px]">
          ...
          {{ dayjs(data.created_at).format("YYYY.M.D HH:mm") }}
        </div>
        ...
        <div class="flex flex-col gap-[24px] md:hidden">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item @click="quote(comment.content)">
                      ...
                      <span class="ml-[4px]">{{ $t('dailyPaper.reply') }}</span>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
          ...
          <div>
            <el-button type="primary" @click="create">{{ $t('dailyPaper.create') }}</el-button>
            <el-button @click="cancel">{{ $t('dailyPaper.cancel') }}</el-button>
          </div>
        </div>
      </div>
      <div class="p-[16px]">
        <div class="flex gap-[8px] items-center mb-[16px]">
          ...
          <div class="text-[16px] font-[500] leading-[24px] text-[#303133]">{{ $t('dailyPaper.model') }}</div>
          <div class="text-[16px] font-[400] leading-[24px] text-[#909399]">{{ data.model_links.length }}</div>
        </div>
        <div class="flex flex-col gap-[16px]">
          <ModelItem v-for="model in modelList" :key="model.path"  :model="model"></ModelItem>
        </div>
        <div class="flex gap-[8px] items-center mt-[48px] mb-[16px]">
          <dataset-icon width="18px" height="18px"/>
          <div class="text-[16px] font-[500] leading-[24px] text-[#303133]">{{ $t('dailyPaper.dataset') }}</div>
          <div class="text-[16px] font-[400] leading-[24px] text-[#909399]">{{ data.dataset_links.length }}</div>
        </div>
        <div class="flex flex-col gap-[16px]">
        ...
        <div v-if="data.video_link!=''&&data.video_link!=nil" class="flex gap-[8px] items-center mt-[48px] mb-[16px]">
          <div class="text-[16px] font-[500] leading-[24px] text-[#303133]">{{ $t('dailyPaper.video') }}</div>
        </div>
        <div v-if="data.video_link!=''&&data.video_link!=nil" class="max-w-[480px] flex flex-col gap-[16px]">
          <video ref="videoPlayer" style="width: 100%" controls muted>
            <source :src="data.video_link" type="video/mp4" />
          </video>
        </div>
        <div class="flex flex-col gap-[24px] hidden mt-[32px] md:block">
          <div class="flex gap-[8px] items-center mb-[24px]">
            <div class="px-[8px] py-[8px] bg-[#F0F3FF] rounded-[8px]"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"><path opacity="0.12" d="M9 13.2C9 12.0799 9 11.5198 9.21799 11.092C9.40973 10.7157 9.71569 10.4097 10.092 10.218C10.5198 10 11.0799 10 12.2 10H18.8C19.9201 10 20.4802 10 20.908 10.218C21.2843 10.4097 21.5903 10.7157 21.782 11.092C22 11.5198 22 12.0799 22 13.2V20.3636C22 21.357 20.8103 21.8669 20.0909 21.1818C19.3906 20.5149 18.4606 20.1429 17.4935 20.1429H12.2C11.0799 20.1429 10.5198 20.1429 10.092 19.9249C9.71569 19.7331 9.40973 19.4272 9.21799 19.0508C9 18.623 9 18.063 9 16.9429V13.2Z" fill="#4D6AD6"/><path d="M9 14H7.25C7.00285 14 6.87928 14 6.75895 14.0097C6.27553 14.0487 5.80877 14.2043 5.39866 14.4631C5.29658 14.5276 5.19772 14.6017 5 14.75V14.75C4.48857 15.1336 4.23286 15.3254 4.03782 15.4003C3.21483 15.7164 2.29801 15.258 2.05709 14.4099C2 14.2089 2 13.8893 2 13.25V6.8C2 5.11984 2 4.27976 2.32698 3.63803C2.6146 3.07354 3.07354 2.6146 3.63803 2.32698C4.27976 2 5.11984 2 6.8 2H13.2C14.8802 2 15.7202 2 16.362 2.32698C16.9265 2.6146 17.3854 3.07354 17.673 3.63803C18 4.27976 18 5.11984 18 6.8V7V7.5M20.0909 21.1818V21.1818C20.8103 21.8669 22 21.357 22 20.3636V13.2C22 12.0799 22 11.5198 21.782 11.092C21.5903 10.7157 21.2843 10.4097 20.908 10.218C20.4802 10 19.9201 10 18.8 10H12.2C11.0799 10 10.5198 10 10.092 10.218C9.71569 10.4097 9.40973 10.7157 9.21799 11.092C9 11.5198 9 12.0799 9 13.2V16.9429C9 18.063 9 18.623 9.21799 19.0508C9.40973 19.4272 9.71569 19.7331 10.092 19.9249C10.5198 20.1429 11.0799 20.1429 12.2 20.1429H17.4935C18.4606 20.1429 19.3906 20.5149 20.0909 21.1818Z" stroke="#4D6AD6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></div>
            {{ $t('dailyPaper.discuss') }}
          </div>
          <div class="border border-[#EBEEF5] border-b-[2px] rounded-[4px] mb-[24px]" v-for="comment in commentData" :key="comment.id">
            <div class="flex justify-between gap-[4px] border-b border-[#EBEEF5] items-center px-[12px] py-[9px] bg-[#F5F7FA]">
              <div class="flex items-center gap-[8px]">
                <el-avatar :size="24" :src="comment.user.avatar"></el-avatar>
                {{comment.user.name}} {{comment.time}}
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        backUrl: `/daily_papers${window.location.search}`,
    ...
    methods: {
      ...
      async create(){
        if (this.desc === '') {
          ElMessage({ message: this.$t('dailyPaper.warn'), type: "warning" });
          return;
        }
        this.createComment(this.dailyPaper.id).then((data) => {
          ElMessage({ message: this.$t('dailyPaper.success'), type: "success" });
          this.getComment(this.dailyPaper.id)
        })
        .catch(err => {
      ...
      async createComment(dailyPaperId){
        const commentCreateEndpoint = "/internal_api/comments"
        const newComment = {
          content: this.desc,
        };
        const commentJsonData = {
          commentable_type:'DailyPaper',
          commentable_id:dailyPaperId,
          comment:newComment
        }
        const commentOption = {
          method:'POST',
