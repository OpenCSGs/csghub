    ...
    <div class="mt-9">
      ...
      >
        <div class="w-full flex md:flex-col gap-[16px] items-center">
          <el-form-item class="w-full" :label="t('endpoints.new.name')" prop="name">
            <el-input v-model="dataForm.name" :placeholder="t('all.pleaseInput', { value: t('endpoints.new.name') })" input-style="width: 100%" >
              <template #suffix>            
          ...
          <el-form-item :label="t('endpoints.new.modelId')" class="w-full" prop="model_path">
            <el-autocomplete
              clearable
        ...
        <div class="w-full flex md:flex-col gap-[16px] items-center">
          <el-form-item :label="t('endpoints.new.minReplica')" class="w-full" prop="min_replica">
            <el-select
              v-model="dataForm.min_replica"
              :placeholder="t('all.pleaseSelect', { value: t('endpoints.new.minReplica') })"
              size="large"
              style="width: 100%"
            >
              <el-option
          ...
          <el-form-item :label="t('endpoints.new.maxReplica')" class="w-full" prop="max_replica">
            <el-select
              v-model="dataForm.max_replica"
              :placeholder="t('all.pleaseSelect', { value: t('endpoints.new.maxReplica') })"
              size="large"
              style="width: 100%"
            >
              <el-option
        ...
        <el-form-item :label="t('endpoints.new.cluster')" class="w-full" prop="endpoint_cluster">
          <el-select
            v-model="dataForm.endpoint_cluster"
            :placeholder="t('all.pleaseSelect', { value: t('endpoints.new.cluster') })"
            size="large"
            style="width: 100%"
        ...
        <el-form-item :label="t('endpoints.new.resource')" class="w-full" prop="cloud_resource">
          <el-select
            v-model="dataForm.cloud_resource"
            :placeholder="t('all.pleaseSelect', { value: t('endpoints.new.resource') })"
            size="large"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item :label="t('endpoints.new.framework')" class="w-full" prop="framework_id">
          <el-select
            v-model="dataForm.framework_id"
            :placeholder="t('all.pleaseSelect', { value: t('endpoints.new.framework') })"
            size="large"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item class="w-full">
          <el-radio-group v-model="dataForm.visibility" class="!block" prop="visibility">
            <el-radio class="w-full !border-2 mr-0 mb-[32px] !rounded-xl !h-auto !items-start !p-4" label="public" size="large" border>
              ...
              <p class="whitespace-normal text-[#475467] font-light">{{ t('endpoints.new.publicDesc') }}</p>
            </el-radio>
            <el-radio class="w-full !border-2 mr-0 !rounded-xl !h-auto !items-start !p-4" label="private" size="large" border>
              ...
              <p class="whitespace-normal text-[#475467] font-light">{{ t('endpoints.new.privateDesc') }}</p>
            </el-radio>
          </el-radio-group>
        </el-form-item>
...
<script setup>
  ...
  const rules = ref({
    name: [
      { required: true, message: t('all.pleaseInput', { value: t('endpoints.new.name') }), trigger: 'blur' },
      // limit 2-64 length
    ...
    model_path: [
      { required: true, message: t('all.pleaseInput', { value: t('endpoints.new.modelId') }), trigger: 'blur' }
    ],
    min_replica: [
      { required: true, message: t('all.pleaseSelect', { value: t('endpoints.new.minReplica') }), trigger: 'change' },
    ],
    max_replica: [
      { required: true, message: t('all.pleaseSelect', { value: t('endpoints.new.maxReplica') }), trigger: 'change' },
    ],
  })
  ...
  const fetchResources = async () => {
    ...
    const res = await jwtFetch(`${csghubServer}/api/v1/space_resources?cluster_id=${dataForm.value.endpoint_cluster}`, options);
    if (!res.ok) {
  ...
  const updateRuntimeFramework = async () => {
    const res = await jwtFetch(`${csghubServer}/api/v1/models/${dataForm.value.model_path}/runtime_framework`);
    if (!res.ok) {
      dataForm.value.endpoint_framework = ""
      endpointFrameworks.value = []
    } else {
      res.json().then((body) => {
        dataForm.value.endpoint_framework = body.data == null ? "" : body.data[0].id;
