class ProfileController < ApplicationController
  ...
  def likes
    ...
    @models = csghub_api.get_user_likes(current_user&.name, 'models', { current_user: current_user&.name})
    @datasets = csghub_api.get_user_likes(current_user&.name, 'datasets', { current_user: current_user&.name})
    @spaces = csghub_api.get_user_likes(current_user&.name, 'spaces', { current_user: current_user&.name})
    @codes = csghub_api.get_user_likes(current_user&.name, 'codes', { current_user: current_user&.name})
    @organizations = @user.organizations
    @is_current_user_access = current_user.present? && (current_user == @user)
  end
end
