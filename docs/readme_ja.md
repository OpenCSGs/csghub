...
### 更新
- v0.8.0 大規模リリースで、独立したuser_serverサービスを導入し、ユーザー、組織、トークンの管理をCSGHubからバックエンドサーバーのCSGHub-serverに移動します。アプリケーション空間、モード推論、およびファインチューンの均一なデプロイメントのための独立したstarhub_server_runnerサービスを導入します。リソース管理が強化され、異なる地域の複数のk8sクラスターはサポートされていません。
