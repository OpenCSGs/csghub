  ...
  <div v-if="comment" class="flex border-b border-gray-200 last-of-type:border-0 pb-[18px] mb-[18px]">
    ...
    <div class="w-full">
      <div class="text-[14px] leading-[22px] flex">
        ...
        <button v-if="current_user_id === comment.user.id.toString()" @click="confirmDelete(comment.id)" class="p-1 hover:bg-gray-500 rounded-[4px]">
      ...
      <div class="text-[14px] leading-[23px] text-gray-500">{{ comment.content }}</div>
    </div>
  </div>
</template>
