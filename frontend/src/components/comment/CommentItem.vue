<template>
  <div v-if="comment" class="flex border-b border-gray-200 last-of-type:border-0 pb-[18px] mb-[18px]">
    <div class="mr-2">
      <el-avatar :size="32" :src="comment.user.avatar"></el-avatar>
    </div>
    <div class="w-full">
      <div class="text-sm leading-[22px] flex">
        <div class="text-gray-700 font-medium mr-4">{{ comment.user.name }}</div>
        <div class="text-gray-500 flex-1">{{ comment.time }}</div>
        <button v-if="current_user_id === comment.user.id.toString()" @click="confirmDelete(comment.id)" class="p-1 hover:bg-gray-500 rounded-xs">
      ...
      <div class="text-sm leading-[23px] text-gray-500">{{ comment.content }}</div>
    </div>
  </div>
</template>
