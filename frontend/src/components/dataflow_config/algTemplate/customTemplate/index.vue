  ...
  <div class="text-left w-full h-full pl-8 py-8 overflow-auto bg-white">
    ...
    <div class="mainOption mt-8">
      ...
      <div class="flex items-center justify-between gap-3 xs:block">
        ...
        <CsgButton
          class="btn btn-primary btn-sm whitespace-nowrap"
          @click="router.push(`/datapipelines/createTemplate?url=customize&type=add`)"
          ...
          <template #footer>
            <div class="flex justify-between items-center text-xs">
              ...
              <div class="flex">
                ...
                <div
                  class="mr-[12px] hover:underline cursor-pointer"
                  @click="router.push(`/datapipelines/createTemplate?url=customize&type=edit&templateId=${item.id}`)"
                >
                  {{ t('dataPipelines.edit')}}
                </div>
                <div
                  class="text-brand-600 hover:underline cursor-pointer"
                  @click="router.push('/datapipelines/addDataProcessing?url=customize&templateId=' + item.id)"
                >
                  {{ t('dataPipelines.use') }}
                </div>
              </div>
...
const getTemplatesListFun = async () => {
  ...
  params.append('page_size', searchForm.value.page_size)
