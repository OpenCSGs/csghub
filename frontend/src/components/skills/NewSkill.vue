    ...
    <div class="mt-9">
      ...
      >
        <div class="w-full flex md:flex-col gap-[16px] items-center">
          ...
          <el-form-item
            class="w-full"
            :label="t('skills.newSkill.skillEnName')"
            prop="name"
          >
            <el-input
              v-model.trim="dataForm.name"
              :placeholder="
                t('all.pleaseInput', { value: t('skills.newSkill.skillEnName') })
              "
        ...
        >
          <el-form-item
            class="w-full"
            :label="t('skills.newSkill.skillNickName')"
            prop="nickname"
          >
            <el-input
              v-model="dataForm.nickname"
              :placeholder="
                t('all.pleaseInput', { value: t('skills.newSkill.skillNickName') })
              "
            />
          </el-form-item>
          <el-form-item
        ...
        <el-form-item
          class="w-full"
          :label="t('skills.newSkill.skillDesc')"
          prop="desc"
        >
          <el-input
            ...
            type="textarea"
            :placeholder="
              t('all.pleaseInput', { value: t('skills.newSkill.skillDesc') })
            "
          />
        </el-form-item>
        <el-form-item class="w-full">
          <PublicAndPrivateRadioGroup
            ...
            :publicDesc="t('skills.newSkill.publicDesc')"
            :privateDesc="t('skills.newSkill.privateDesc')"
          />
        </el-form-item>
        <p
          class="mb-[18px] rounded-md bg-brand-25 text-brand-500 text-[13px] py-[9px] px-4"
        >
          {{ t('skills.newSkill.tips') }}
        </p>
        <div class="flex justify-end">
          <el-form-item>
            <CsgButton
              ...
              :name="t('skills.newSkill.createSkill')"
              @click="handleSubmit"
            />
          </el-form-item>
        </div>
...
<script setup>
  ...
  import { showContextMessage } from '@/packs/contextMessage.js'
  ...
  const rules = ref({
    owner: [
      {
        required: true,
        message: t('all.pleaseSelect', { value: t('skills.newSkill.owner') }),
        trigger: 'change'
      }
    ],
    name: [
      {
        required: true,
        message: t('all.pleaseInput', { value: t('skills.newSkill.skillName') }),
        trigger: 'blur'
      },
      // limit 2-64 length
      {
    ...
    license: [
      {
        required: true,
        message: t('all.pleaseSelect', { value: t('skills.newSkill.license') }),
        trigger: 'change'
      }
    ]
  })
  ...
  const createSkill = async () => {
    ...
    const newEndpoint = '/skills'
    const { data, error } = await useFetchApi(newEndpoint, options).post().json()
    if (data.value) {
      ElMessage({
        message: t('skills.newSkill.createSuccess'),
        type: 'success'
      })
      window.location.href = `/skills/${data.value.data.path}`
    } else {
      showContextMessage(error.value, t)
    }
  }
</script>
