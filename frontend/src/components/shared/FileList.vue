<template>
  <div class="min-h-[300px] py-8 md:px-5">
    <div class="flex items-center justify-between">
      ...
      <div v-if="lastCommit" class="flex items-center text-sm text-gray-500">
    ...
    <div v-if="files" v-for="file in files" class="flex items-center justify-between px-3 py-2 border border-t-0 border-gray-200 last-of-type:rounded-b-[4px]">
      ...
      <div class="flex items-center justify-end pr-3 text-sm text-gray-500 flex-shrink-0 text-right w-[20%] md:w-auto md:pr-0">
        <span v-if="file.type === 'file'">{{ formatBytes(file.size) }}</span>
        <span v-if="file.lfs" class="flex items-center gap-1 text-xs text-gray-700 ml-2 rounded px-1 border border-gray-500">
