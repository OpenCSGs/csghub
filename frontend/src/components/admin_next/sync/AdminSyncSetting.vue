<template>
  <Container title="多源同步" :breadcrumbs="[{ text: '多源同步' }]">
    <div>
      ...
      <el-form :model="form" label-width="auto" label-position="left" style="max-width: 400px">
        ...
        <el-form-item :label="$t('admin.syncSetting.concurrent')">
          <el-select v-model="form.concurrent_count">
            <el-option v-for="count in concurrentCounts" :key="count" :label="count" :value="count"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('admin.syncSetting.bandwidth')">
          <el-select v-model="form.max_bandwidth">
            <el-option v-for="count in bandwidthCounts" :key="count" :label="count" :value="count"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
      ...
      <el-table v-if="queues.length > 0" :data="queues" style="width: 100%">
        <el-table-column prop="path" label="Path"></el-table-column>
        <el-table-column prop="repo_type" label="Type" width="100"></el-table-column>
      ...
      <el-pagination v-model:current-page="page" :page-size="per" layout="prev, pager, next" :total="total"
        @current-change="fetchSyncRecords" class="my-[52px] flex justify-center" />
    </div>
  </Container>
</template>
...
import Container from '../admin_component/Container.vue'
