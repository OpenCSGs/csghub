          ...
          <div class="flex justify-between gap-4">
            <div class="flex-1">
              <el-tabs
                ...
                class="log-tabs">
                <el-tab-pane
                  :label="$t('application_spaces.errorPage.build')"
                  name="build" />
                <el-tab-pane
                  :label="$t('application_spaces.errorPage.container')"
                  name="container" />
              </el-tabs>
            </div>
            <CsgButton
...
<script setup>
  ...
  const scrollToBottom = () => {
    const targetDiv = isBuildLogTab.value
      ? buildLogDiv.value
      : containerLogDiv.value
  ...
  watch(spaceLogsDrawer, (newVal) => {
    if (newVal) {
      nextTick(() => {
        const logContainer = isBuildLogTab.value
          ? buildLogDiv.value
          : containerLogDiv.value
        if (logContainer) {
      ...
      buildLogBlocks.value = []
      containerLogBlocks.value = []
  ...
  watch(activeTab, () => {
    if (spaceLogsDrawer.value) {
      nextTick(() => {
        ...
        if (logContainer) {
          // 移除旧容器的事件监听
          const oldContainer = !isBuildLogTab.value
            ? buildLogDiv.value
            : containerLogDiv.value
          if (oldContainer) {
  ...
  const syncSpaceLogs = () => {
    ...
    fetchEventSource(
      `${csghubServer}/api/v1/spaces/${props.namespace}/${
        props.repoName
      }/logs?since=${timeDuration.value == 'all' ? '' : timeDuration.value}`,
      {
        openWhenHidden: true,
        headers: {
        ...
        onmessage(ev) {
          ...
          if (ev.event === 'Build') {
            appendLog(buildLogDiv, ev.data, buildLogLineNum, buildLogBlocks, {
              value: lastBuildLogTime
            })
            nextTick(() => {
              scrollToBottom()
            })
  ...
  const appendLog = (
    ...
    function addSpaceAfterLogLevel(text) {
      return text.replace(
        /(INFO|WARN|ERROR|DEBUG|TRACE|FATAL)([^\s])/g,
        '$1 $2'
      )
    }
    ...
    processedData.split('\n').forEach((line) => {
      ...
      line = line
        .replace(
          /\[(\d{4})\]/g,
          (_, time) => `[${time.slice(0, 2)}:${time.slice(2)}]`
        )
    ...
    refLineNum.value = logBlocksRef.value.reduce(
      (acc, b) => acc + b.lines.length,
      0
    )
    nextTick(() => {
      scrollToBottom()
    ...
    logBlocksRef.value.forEach((block) => {
      ...
      block.lines.forEach((line) => {
        ...
        lineHtml = lineHtml.replace(
          /^(<span[^>]*>)*(INFO|WARN|ERROR|DEBUG|TRACE|FATAL|NOTICE|WARNING)(<\/span[^>]*>)*([\s\S]*?)(?=<|$)/,
          (match, p1, type, p3, rest) => {
            return `${p1 || ''}${type}${
              p3 || ''
            }<span style='display:inline-block;width:4px'></span>${rest || ''}`
          }
        )
  ...
  const downloadLog = () => {
    ...
    const logContent = logBlocks.value
      .map((block) => {
        // 转换时间格式 [0004] → [00:04]
          ...
          block.lines
            .map((line) => {
              ...
              plainText = plainText.replace(
                /^(INFO|WARN|ERROR|DEBUG|TRACE|FATAL|NOTICE|WARNING)([^\s])/,
                '$1 $2'
              )
    ...
    try {
      ...
      link.download = `${props.repoName}_${
        isBuildLogTab.value ? 'build' : 'container'
      }_${new Date().toISOString().slice(0, 10)}.log`
  ...
  onMounted(async () => {
    ...
    setRepoTab({
      ...
      currentBranch:
        props.currentBranch || repoDetailStore.defaultBranch || 'main'
    })
  })
