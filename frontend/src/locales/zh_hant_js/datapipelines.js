export const dataPipelines = {
  ...
  "specialCharacterRatioFiltering": "特殊字符占比過濾",
  ...
  "windowLength": "窗口長度",
  "description": "描述",
  "textNormalizationDesc": "文本Unicode標準化和繁體轉簡體",
  ...
  "specialCharacterRatioFilteringDesc": "根據特殊字符占比過濾文本，保留特殊字符個數占文本總長度比例不超過設定閾值的樣本，特殊字符包括標點符號，數字，空格符號，emoji表情包等，超過設定比例的數據樣本將被過濾",
  ...
  "quality_classifier_common_internal_dec": "本質量分類器類用於預測數據集中文檔的評分。它將計算所有行的分數，並為每行提供兩列：分數（score）和是否保留（should_keep），以幫助用戶決定應該刪除哪一行。默認情況下，如果分數高於 0.9，則將該行標記為 should_keep=1。",
